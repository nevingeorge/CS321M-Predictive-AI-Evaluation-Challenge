...
ENCODER = None   # SentenceTransformer


if _local_smoke_test_enabled():
    print("[model_v10] Skipped loading for local smoke test.", flush=True)
else:
    try:
        import numpy as np
        import torch
    ...
    try:
        ...
        # Load using model name + local_files_only — same approach as v5 which worked.
        ...
        print(
            f"[model_v10] Loaded. {len(SUBJECT_INDEX)} subjects. "
            f"w_factor={W_FACTOR:.3f} w_llm={W_LLM:.3f} bias={W_BIAS:.3f}",
            flush=True,
        )
    except Exception as exc:
        raise RuntimeError(f"Failed to load v10 model: {exc}") from exc

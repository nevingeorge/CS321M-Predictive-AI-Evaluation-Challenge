...
import os
# Force ALL HuggingFace libraries offline before any of them are imported.
# Use direct assignment (not setdefault) so platform pre-set values are overridden.
...
def _find_hf_snapshot(model_id: str, cache_dir: str | None) -> str:
    """Locate a pre-downloaded HF model snapshot using only filesystem operations.

    No huggingface_hub import, no network calls. Raises RuntimeError if not found.
    HF cache structure: {cache_dir}/hub/models--{org}--{name}/snapshots/{commit}/
    """
    ...
    if hf_home:
        search_roots += [Path(hf_home) / "hub", Path(hf_home)]
    search_roots += [
        Path("/app/hf_cache") / "hub",
        Path("/app/hf_cache"),
        Path.home() / ".cache" / "huggingface" / "hub",
    ]
    for root in search_roots:
        model_dir = root / dir_name
        # Try refs/main → snapshots/{commit}
        ...
        if refs.exists():
            ...
            if snap.is_dir():
                return str(snap)
        # Fall back: first available snapshot
        snaps_dir = model_dir / "snapshots"
        if snaps_dir.is_dir():
            entries = [s for s in snaps_dir.iterdir() if s.is_dir()]
            if entries:
                return str(entries[0])
    raise RuntimeError(
        f"Model '{model_id}' not found in local HF cache. "
        f"Ensure it is declared in models.txt so the platform pre-downloads it."
    )


# Encoder globals (transformers AutoModel, no sentence_transformers)
_ENCODER_MODEL = None
_ENCODER_TOK = None
...
def _encode(text: str) -> "torch.Tensor":
    ...
    ).to(next(_ENCODER_MODEL.parameters()).device)
    with torch.no_grad():
        out = _ENCODER_MODEL(**inputs)
...
if _local_smoke_test_enabled():
    print("[model_v9] Skipped loading for local smoke test.", flush=True)
else:
    try:
        import numpy as np
        import torch
    ...
    try:
        ...
        _here = Path(__file__).parent   # artifact files bundled in the ZIP

        # Locate model snapshots via pure filesystem lookup — zero network calls.
        _encoder_path = _find_hf_snapshot("BAAI/bge-large-en-v1.5", _cache)
        _qwen_path = _find_hf_snapshot("Qwen/Qwen3.5-9B", _cache)

        # Sentence encoder — use transformers AutoModel directly (no sentence_transformers)
        _ENCODER_TOK = AutoTokenizer.from_pretrained(_encoder_path)
        _ENCODER_MODEL = AutoModel.from_pretrained(_encoder_path)
        _ENCODER_MODEL.eval()
        if torch.cuda.is_available():
            _ENCODER_MODEL = _ENCODER_MODEL.to("cuda")
        ...
        # LLM judge — loaded from local snapshot path
        LLM_TOKENIZER = AutoTokenizer.from_pretrained(_qwen_path)
        LLM = AutoModelForCausalLM.from_pretrained(
            _qwen_path, torch_dtype=torch.bfloat16, device_map="auto"
        )
        LLM.eval()
        ...
        print(
            f"[model_v9] Loaded. {len(SUBJECT_INDEX)} subjects. "
            f"w_factor={W_FACTOR:.3f} w_llm={W_LLM:.3f} bias={W_BIAS:.3f}",
            flush=True,
        )
    except Exception as exc:
        raise RuntimeError(f"Failed to load v9 model: {exc}") from exc
...
def _compute_factor_logit(input: dict) -> float:
    ...
    enc = _encode(item_text)   # uses transformers AutoModel, no sentence_transformers
    with torch.no_grad():
        out = ITEM_HEAD(enc)
...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    if _ENCODER_MODEL is None and LLM is None:
        return 0.5

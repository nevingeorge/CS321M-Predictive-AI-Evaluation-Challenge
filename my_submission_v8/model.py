...
if _local_smoke_test_enabled():
    ...
    try:
        ...
        # Resolve local snapshot directories — no network calls with local_files_only=True.
        # Loading from an explicit local path bypasses all Hub model-ID validation,
        # which is what causes the outbound network calls.
        _encoder_path = snapshot_download(
            "BAAI/bge-large-en-v1.5", cache_dir=_cache, local_files_only=True
        )
        _qwen_path = snapshot_download(
            "Qwen/Qwen3.5-9B", cache_dir=_cache, local_files_only=True
        )

        # Factor model — load encoder from local path (no Hub validation)
        ENCODER = SentenceTransformer(_encoder_path)
        ...
        # LLM judge — load from local path (no Hub validation)
